...
type alias OrderTipes = { balance: (Maybe Float, String), amount: (Maybe Float, String),}
type alias Model = {entries: List Entry, filter: Filter}
    ...
    case msg of
        ...
        UpdateSongFilter newFilter -> 
            let
                ...
                newFilters = {  allFilters | songFilter = newFilter }
            in
                { model | filter=newFilters }
        UpdateArtistFilter newFilter -> 
            let
                allFilters = model.filter
                newFilters = {  allFilters | artistFilter = newFilter }
            in
                { model | filter=newFilters }
        UpdateStartYearFilter newFilter -> 
            let
                allFilters = model.filter
                newFilters = {  allFilters | startYearFilter = Maybe.withDefault 0 <| String.toInt newFilter}
            in
                { model | filter=newFilters }
        UpdateEndYearFilter newFilter -> 
            let
                allFilters = model.filter
                newFilters = {  allFilters | endYearFilter =  Maybe.withDefault 0 <| String.toInt newFilter}
            in
                { model | filter=newFilters }
